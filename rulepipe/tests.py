...
class TestRulepipe(MyTestCase):
    ...
    def test_2_add_rule_with_missing_body(self):
        ...
        self.assertEqual(
            response.json,
            {"err": "At least one 'type' value required to add new rule."},
        )
    ...
    def test_5_add_exist_rule(self):
        _rule = {"type": "rule"}
        response = self.simulate_post("/add_rule/AwesomeRule", json=_rule)
        ...
        self.assertEqual(
            response.json,
            {
                "err": "Couldn't create new rule, rule named by AwesomeRule already exists."
            },
        )

    def test_6_delete_rule(self):
        response = self.simulate_delete("/rules/AwesomeRule")

        self.assertEqual(response.json, {"msg": True})

    def test_7_execute_undefined_rule(self):
        response = self.simulate_post("/rules/ThisRuleNeverDefined", json={})

        self.assertEqual(response.status_code, 404)
        self.assertEqual(
            response.json, {"err": "Rule 'ThisRuleNeverDefined' not found."}
        )
